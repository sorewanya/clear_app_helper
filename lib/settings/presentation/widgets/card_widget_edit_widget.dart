...
class CardWidgetEditWidget extends StatelessWidget {
  ...
  const CardWidgetEditWidget({super.key, required this.listOfNames, required this.routeName});

  @override
  Widget build(BuildContext context) {
    List<SettingsEntity> listOfSettings = listOfNames
        .map((e) => context.read<SettingsBloc>().getByNamed(e))
        .whereType<SettingsEntity>()
        .toList();
    List<Widget> listOfWidget = [];
    for (SettingsEntity item in listOfSettings) {
      listOfWidget.add(Text("${GetIt.instance<CoreI18n>().widgetEditPartsIn} ${item.name.split(',').last}:"));
      listOfWidget.add(
        ListOfValuesWidget(
      ...
      listOfWidget.add(const SizedBox(height: 5));
    }
    listOfWidget.add(
      TextButton(
